      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-10 h-10 bg-gradient-secondary rounded-lg flex items-center justify-center font-bold text-secondary-foreground" aria-label="Smash Point logo">FH</div>
            <span className="text-xl font-bold text-foreground">Food House</span>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            {isSupabaseConfigured && user && <div className="hidden md:flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            <Button variant="outline" size="icon" onClick={onCartOpen} className="relative btn-outline-yellow" aria-label="Abrir carrinho">
              ...
              {cartItemsCount > 0 && <span className="absolute -top-2 -right-2 bg-secondary text-secondary-foreground text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center" aria-live="polite">
                  {cartItemsCount}
                </span>}
            </Button>
        ...
        {isMenuOpen && <div className="md:hidden mt-4 pb-4 border-t border-border pt-4">
            <nav className="flex flex-col space-y-3" aria-label="Navegação mobile">
              <button className="text-left text-foreground hover:text-secondary transition-colors font-medium py-2" onClick={() => {
            onViewMenu();
            setIsMenuOpen(false);
